...
import com.study.backend.enity.descriptors.ManagerDescriptor;
...
//        };
    ...
    public String dumpData(FilterDto filter) throws Exception {
        ...
        SQLQuery<Manager> q = queryFactory.query().select(projection).from(managers);
        q = FilterHeper.makeFilterQuery(q, ManagerDescriptor.fieldDescriptorMap, filter);

        return PrinterFactory.getDumperFor(Manager.class)
                             .dumpData(q);
    }
